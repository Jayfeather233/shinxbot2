...
private:
    std::map<std::string,
             std::vector<std::tuple<fs::path, fs::path, std::string>>>
        arc_list; // pair<file, rele_path, passwd>
    std::string default_pwd;

    bool archive_add_path(zip_t *archive, const fs::path &path,
                          const std::string &passwd,
                          const fs::path &rele_path = fs::path());

    bool archive_add_dir(zip_t *archive, const fs::path &path,
                         const std::string &passwd,
                         const fs::path &rele_path = fs::path());

    bool archive_add_file(zip_t *archive, const fs::path &path,
                          const std::string &passwd,
                          const fs::path &rele_path = fs::path());
...
public:
     ...
     */
    void add_path(const std::string &name, const fs::path &path,
                  const fs::path &rele_path = fs::path(),
                  const std::string &passwd = "");
