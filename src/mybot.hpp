...
private:
    ...
    void get_log()
    {
        ...
        oss << "./log/" << botqq << "/"
            << tt.tm_year + 1900 << '_' << std::setw(2) << std::setfill('0')
        ...
        LOG_output[0] =
            std::ofstream(oss.str() + "/info.log", std::ios_base::app);
        LOG_output[1] =
            std::ofstream(oss.str() + "/warn.log", std::ios_base::app);
        LOG_output[2] =
            std::ofstream(oss.str() + "/erro.log", std::ios_base::app);
    }
...
public:
    mybot(int recv_port, int send_port) : bot(recv_port, send_port){}

    bool is_op(const int64_t a) const { return op_list.find(a) != op_list.end(); }
        ...
        if ((post_type == "request" || post_type == "notice") && bot_isopen) {
            ...
            if (J.isMember("message_type") && J.isMember("message")) {
                ...
                if (message_type == "group" || message_type == "private") {
                    ...
                    msg_meta conf =
                        (msg_meta){message_type, user_id, group_id, message_id, this};
                    if (message == "bot.help") {
    ...
    std::string cq_send(const std::string &message, const msg_meta &conf) const
    {
        Json::Value input;
    ...
    std::string cq_get(const std::string &end_point) const
    {
