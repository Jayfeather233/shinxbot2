...
void gray_list::process(std::string message, const msg_meta &conf)
{
    ...
    Json::ArrayIndex ind;
    if ((ind = json_array_find(g_list[conf.group_id], user_id)) !=
        g_list[conf.group_id].size()) {
        Json::Value Jdata;
        Jdata["group_id"] = conf.group_id;
        Jdata["user_id"] = user_id;
        Json::Value res =
            string_to_json(conf.p->cq_send("set_group_kick", Jdata));
...
std::string gray_list::help() { return "群灰名单：使用 加入灰名单 @xxx 使用"; }

DECLARE_FACTORY_FUNCTIONS(gray_list)