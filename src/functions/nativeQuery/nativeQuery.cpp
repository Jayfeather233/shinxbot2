...
void nativeQuery::process(std::string message, const msg_meta &conf)
{
    ...
    try {
        ...
        conf.p->cq_send(cq_encode(string_to_json(response).toStyledString()),
                        conf);
    }
    catch (std::exception &e) {
        conf.p->setlog(LOG::ERROR,
                       std::string("nativeQuery: exception: ") + e.what());
...
std::string nativeQuery::help(const msg_meta &conf, help_level_t level)
{
    ...
    return "nativeQuery: OP-only raw OneBot API call\n"
           ".nq endpoint {json payload}";
}
