...
static std::string help_url =
    "https://demo.hedgedoc.org/zSOS-JMKSCey2E_4UjrrOQ?view";
...
void NGGame::send_vic(const msg_meta &conf)
{
    for (auto it : ng) {
        send_msg_ng(conf.p, 0, it.first,
                    "Pls set NG word for " + get_username(conf.p,
                                                          it.second->nex->id,
                                                          conf.group_id));
    }
}
...
std::string NGGame::overall(const msg_meta &conf)
{
    ...
    for (auto it : ng) {
        if (is_alive(it.first)) {
            content = content + get_username(conf.p, it.first, conf.group_id) +
                      ": alive\n";
        }
        else {
            content = content + get_username(conf.p, it.first, conf.group_id) +
...
{
    if (state != gameState::work || ng.find(user_id) == ng.end() ||
        !ng[user_id]->alive ||
...
{
    if (cq.find("[CQ:at,qq=") == std::string::npos ||
        cq.find("]") == std::string::npos) {
        return 0;
    }
    size_t st = cq.find("[CQ:at,qq=") + 10;
    size_t ed = cq.find("]", st);
...
void NGgame::process(std::string message, const msg_meta &conf)
{
    ...
    std::string prefix =
        "[CQ:at,qq=" + std::to_string(conf.p->get_botqq()) + "] ng ";
    if (conf.message_type == "group") {
        ...
        if (message.find(prefix + "guess") != std::string::npos) // guess
        {
            ...
            if (game.check_ng(guess, uid)) {
                send_msg_ng(conf.p, gid, 0,
                            "Congrats! Your NG word is " + guess);
                send_msg_ng(conf.p, gid, 0,
                            "Game over! The winner is " +
                                get_username(conf.p, uid, gid));
                game.abort();
            }
            else {
                send_msg_ng(conf.p, gid, 0,
                            "Oops, your NG word is " + game.get_ng(uid) +
                                "\nGood luck next time!");
                game.lose(uid);
                if (game.check_end()) {
                    send_msg_ng(
                        conf.p, gid, 0,
                        "Game over! The winner is " +
                            get_username(conf.p, game.get_winner(), gid));
                    game.abort();
                }
            }
        }
        else if(game.get_state() == gameState::work) // normal check
        {
            ...
            if (game.check_ng(expanded_msg, uid)) {
                send_msg_ng(conf.p, gid, 0,
                            get_username(conf.p, uid, gid) +
                                " Out! The NG word is " + game.get_ng(uid));
                game.lose(uid);
                if (game.check_end()) {
                    send_msg_ng(
                        conf.p, gid, 0,
        ...
        if (message.find(prefix + "start") != std::string::npos) {
            ...
            case gameState::idle: {
                ...
                send_msg_ng(conf.p, gid, 0,
                            "Game created, send @Bot ng join to join\nHelp: " +
                                help_url);
                break;
            }
            case gameState::join: {
                if (game.total_cnt() <= 1) {
                    ...
                    game.link();
                    send_msg_ng(conf.p, gid, 0,
                                "Pls send the NG words via private chats");
                    game.send_vic(conf);
                }
                break;
            }
            case gameState::init: {
                if (game.ready_cnt() == game.total_cnt()) {
                    ...
                    send_msg_ng(conf.p, gid, 0,
                                "Game starts! Pls check your private chat for "
                                "the NG words");
                    game.send_list(conf);
                }
                else {
            ...
            case gameState::join: {
                send_msg_ng(conf.p, gid, 0,
                            "A game is collecting players, currently " +
                                std::to_string(game.total_cnt()) + " player" +
                                (game.total_cnt() > 1 ? "s" : ""));
                break;
            }
            case gameState::init: {
                send_msg_ng(conf.p, gid, 0,
                            "A game is collecting NG words (" +
                                std::to_string(game.ready_cnt()) + "/" +
                                std::to_string(game.total_cnt()) + ")");
                send_msg_ng(conf.p, 0, uid,
                            "Your victim is " +
                                get_username(conf.p, game.get_vic(uid), gid));
                break;
            }
            case gameState::work: {
            ...
            if (game.get_state() == gameState::idle) {
                send_msg_ng(
                    conf.p, gid, 0,
                    "No existed game. Send @Bot ng start to create one");
            }
            else if (game.get_state() != gameState::join) {
                send_msg_ng(conf.p, gid, 0,
                            "Pls wait for the ongoing game to terminate");
            }
            ...
            if (game.quit(victim)) {
                send_msg_ng(conf.p, gid, 0,
                            get_username(conf.p, victim, gid) +
                                " left the game\nSend @Bot ng state to check "
                                "the updated NG words");
                if (game.check_end()) {
                    send_msg_ng(
                        conf.p, gid, 0,
                        "Game over! The winner is " +
                ...
                send_msg_ng(conf.p, gid, 0,
                            "Currently you are not in this game");
            }
        }
        ...
        for (auto g : games) {
            ...
            if (game.get_state() == gameState::init) {
                ...
                if (vic) {
                    ...
                    send_msg_ng(conf.p, 0, uid,
                                "Set NG word for" +
                                    get_username(conf.p, vic, g.first) + ": " +
                                    message);
                    send_msg_ng(conf.p, g.first, 0,
                                get_username(conf.p, uid, g.first) +
                                    " has set the NG word!(" +
                                    std::to_string(game.ready_cnt()) + "/" +
                                    std::to_string(game.total_cnt()) + ")");
                }
...
bool NGgame::check(std::string message, const msg_meta &conf) { return true; }

std::string NGgame::help() { return "NG Game Help: " + help_url; }
