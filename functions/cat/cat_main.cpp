...
catmain::catmain()
{
    ...
    if (ans != "") {
        ...
        setlog(LOG::ERROR, "Missing file: ./data/cat.json");
    }
...
void catmain::process(std::string message, const msg_meta &conf)
{
    ...
    if (message.find(".adopt") == 13) {
        ...
        auto it = cat_map.find(conf.user_id);
        if (it == cat_map.end()) {
            cq_send("You don't have one!", conf);
            return;
        }
        else {
            ...
            if (name.length() <= 0) {
                ...
                cq_send(it->second.rename(name), conf);
                save_map();
            }
        }
    }
        ...
        if (it == cat_map.end()) {
            ...
            cq_send(it->second.process(message), conf);
        }
    }
}
