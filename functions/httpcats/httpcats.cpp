...
void httpcats::process(shinx_message msg){
    ...
    setlog(LOG::INFO, "httpcats at group " + std::to_string(msg.group_id) + " by " + std::to_string(msg.user_id));
    msg.message = "[CQ:image,file=https://httpcats.com/" + std::to_string(code) + ".jpg,id=40000]";
    cq_send(msg);
}
