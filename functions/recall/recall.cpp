...
bool recall::check(shinx_message msg){
    return (msg.message.find("[CQ:reply,id=") != msg.message.npos && msg.message.find("recall") != msg.message.npos);
}
std::string recall::help(){
    return "撤回消息：回复某句话输入recall";
}